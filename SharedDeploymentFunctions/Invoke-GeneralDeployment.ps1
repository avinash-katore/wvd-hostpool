function Invoke-GeneralDeployment {
  ...
  process {
    ...
    $templateUri = 'https://raw.githubusercontent.com/avinash-katore/wvd-hostpool/master/master/Modules/ARM/{0}/deploy.json' -f $moduleName
    ...
    switch -regex ($deploymentSchema) {
      ...
      if ($moduleName -eq "WvdHostPools")
        {
          Write-Verbose "moduleName '$moduleName'  location '$Location'" -Verbose
          Write-Verbose "DeploymentInputs '$DeploymentInputs'" -Verbose
          $DeploymentInputs["Location"] = "eastus"
        }

      if ($moduleName -eq "WvdApplicationGroups")
        {
          Write-Verbose "moduleName '$moduleName'  location '$Location'" -Verbose
          Write-Verbose "DeploymentInputs '$DeploymentInputs'" -Verbose
          $DeploymentInputs["Location"] = "eastus"
		  ...
		  if ($moduleName -eq "WvdWorkspaces")
        {
          Write-Verbose "moduleName '$moduleName'  location '$Location'" -Verbose
          Write-Verbose "DeploymentInputs '$DeploymentInputs'" -Verbose
          $DeploymentInputs["Location"] = "eastus"
		  ...
		  if ($moduleName -eq "WvdApplications")
        {
          Write-Verbose "moduleName '$moduleName'  location '$Location'" -Verbose
          Write-Verbose "DeploymentInputs '$DeploymentInputs'" -Verbose
          $DeploymentInputs["Location"] = "eastus"
